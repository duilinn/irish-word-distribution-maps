...
import * as volume0001 from '../../public/0001.json';
...
export default function Home() {
  ...
  function getVolume(volumeNumber) {
    fetch("./0002.json")
      ...
      .then((json) => {
        setCurrentVolume(json);
        console.log(currentVolume);
        console.log("getting volume "+volumeNumber);
      });
  };

  for (let part of currentVolume[0].parts)
  {
      for (let item of part.items)
      {
          if ("informants" in item && item.informants.length > 0 &&
          "addressesIreland" in item.informants[0] && item.informants[0].addressesIreland.length > 0)
          {
                ...
                coordsList.push([coords.latitude + randomLatOffset, coords.longitude + randomLonOffset]);
          }
      }
  }
  ...
  return (
    <Layout>
      ...
      <Button onClick={() => getVolume('0002')}>Switch to second volume</Button>
      <Section>
        <Container>
          <Map className={styles.homeMap} width="400" height="400" center={DEFAULT_CENTER} zoom={7}>
            {({ TileLayer, Marker, Popup }) => (
              <>
                ...
                <Marker position={[53,-6]} />
                <Marker position={[51,-10]} />

                {coordsList.map(coords => (
                  <Marker position={coords} key={coords[0]*100+coords[1]}/>
                ))}
              </>
            )}
          </Map>
