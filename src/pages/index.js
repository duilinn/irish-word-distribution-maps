...
import * as volume0001 from '../../public/volumes/0001.json';

const DEFAULT_CENTER = [54, -7];
...
export default function Home() {
  ...
  const [inputTexts, setInputTexts] = useState(["iomarca"]);
  ...
  function searchVolumes() {
    ...
    async function getVolume(volumeNumber) {
      const initialData = await fetch("./volumes_simplified/" + volumeNumber + ".json");
      ...
      for (let item of jsonResponse) {
        if (languagesIncluded[item.language]) {
          ...
          if (foundQueries.length > 0 && item.coordinates.length > 0 && (!isNaN(parseFloat(item.coordinates[0])))) {
            ...
            var markerContent =
              <>
                ...
                {foundQueries.map((f) => (<p>{inputTexts[f]}</p>))}
              </>
            ...
            for (let foundQuery of foundQueries) {
              markerIcon += "<div style='color: " + markerColours[foundQuery] + "; display: inline'>" + markerSymbols[foundQuery] + "</div>";
            }
    ...
    let inputArray = volumeNumbers;//.slice(0, 200);
    (async function () {
      var allCoords = [];
      for (const item of inputArray) {
        //console.log("Fetching volume " + item);
        var fetchedItem = await getVolume(item);
  ...
  return (
    <Layout>
      ...
      <form>
        {inputTexts.map((inputText, i) => (<input
          type="text"
        ...
        <Button disabled={!(inputTexts.length < 5)} onClick={(e) => { e.preventDefault(); if (inputTexts.length < 5) setInputTexts(previousInputTexts => [...previousInputTexts, ""]) }}>+</Button>
        <br />
        <Button onClick={(e) => { e.preventDefault(); searchVolumes() }}>Search</Button>
        <div style={{float: "left"}}>
          Include:
          English
          <input type="checkbox" onChange={(e) => setIncludeEnglish(!includeEnglish)} checked={includeEnglish}/>
          Irish
          <input type="checkbox" onChange={(e) => setIncludeIrish(!includeIrish)} checked={includeIrish}/>
          Mixed
          <input type="checkbox" onChange={(e) => setIncludeMixed(!includeMixed)} checked={includeMixed}/>
      ...
      <table><tbody>
        {inputTexts.map((inputText, i) => (
          <tr style={{ width: "50em", color: markerColours[i]}}><td>{inputText}</td><td>{markerSymbols[i]}</td></tr>
        ))}
      </tbody></table>
      <Section>
        <Container>
          <Map className={styles.homeMap} width="400" height="200" center={DEFAULT_CENTER} zoom={7}>
