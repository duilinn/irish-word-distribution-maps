...
export default function Home() {
  ...
  const [inputText, setInputText] = useState("iomarca");
  ...
  function searchVolumes() {
     ...
     async function getVolume(volumeNumber) {
      ...
      for (let part of jsonResponse[0].parts)
      {
        for (let item of part.items)
        {
          ...
          if (queryFound && "informants" in item && item.informants.length > 0 &&
    ...
    let inputArray = volumeNumbers.slice(0,100);
    (async function() {
      var allCoords = [];
      for(const item of inputArray) {
        //console.log("Fetching volume " + item);
        var fetchedItem = await getVolume(item);
        console.log("Volume " + item + ": " + fetchedItem.length + " points found for "+ inputText);
        allCoords.push([...fetchedItem]);
      }
      console.log("All volumes: " + allCoords.flat(1).length + " points found for "+ inputText);
