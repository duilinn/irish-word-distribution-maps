...
export default function Home() {
  ...
  function searchVolumes() {
     ...
     async function getVolume(volumeNumber) {
      const initialData = await fetch("./volumes/"+volumeNumber+".json");
      const jsonResponse = await initialData.json();
      var tempMarkersList = [];
      var searchQueries = inputTexts;
      //setLastVolumeCalled(volumeNumber);
      for (let part of jsonResponse[0].parts)
      {
        for (let item of part.items)
        {
                ...
                if (page.id == itemPageId) {
                    for (let transcript of page.transcripts) {
                        if (transcript.itemID == item.id) {
                          for (let queryIndex in searchQueries) {
                            var transcriptText = transcript.text.toLowerCase();

                            let searchIndices = [];
                            let index = transcriptText.indexOf(searchQueries[queryIndex]);

                            while (index !== -1) {
                              searchIndices.push(index);
                              index = transcriptText.indexOf(searchQueries[queryIndex], index+1);
                            }
          ...
          if (foundQueries.length > 0 && "informants" in item && item.informants.length > 0 &&
          "addressesIreland" in item.informants[0] && item.informants[0].addressesIreland.length > 0)
          {
            var coords = item.informants[0].addressesIreland[0].coordinates;
            ...
            for (let foundQuery in foundQueries) {
              markerIcon += "<div style='color: "+markerColours[foundQuery]+"; display: inline'>"+markerSymbols[foundQuery]+"</div>";
            }
            tempMarkersList.push({coords: [coords.latitude + randomLatOffset, coords.longitude + randomLonOffset], content: markerContent, icon: markerIcon});
          }
        }
        //console.log(volumeNumber + ": " + tempCoordsList.length);
      }
      //setCoordsList( prevState => [...prevState, ...tempCoordsList]);
    ...
    <Layout>
      ...
      <table><tbody>
        {inputTexts.map((inputText, i) => (
          <tr style={{width: "50em"}}><td>{inputText}</td><td>{markerSymbols[i]}</td></tr>
        ))}
      </tbody></table>
      <Section>
        <Container>
            ...
            {({ TileLayer, Marker, Popup }) => (
              <>
                ...
                {console.log(markersList)}{
                  ...
                  <Marker position={currentMarker.coords} key={i} icon={new L.DivIcon({className: 'my-div-icon',
