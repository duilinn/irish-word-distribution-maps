...
var con = mysql.createConnection({
  host: "localhost",
  user: "root",
  password: "wFQG5Wjas3XYiRm"
});

con.connect(function (err) {
  if (err) throw err;
  console.log("Connected!");
  // con.query("CREATE DATABASE duchas_test", function (err, result) {
  //   if (err) throw err;
  //   console.log("Database created");
  // });
  con.query("USE duchas_test", function (err, result) {
    if (err) throw err;
    ...
    var itemsArray = [];
    for (let volumeNumber of volumeNumbers) {
      const data = fs.readFileSync('./volumes_simplified/' + volumeNumber + '.json', 'utf-8');
      console.log("Adding volume " + volumeNumber);
      ...
      items.forEach((item) => {
        ...
        itemsArray.push([item.title, item.language, lat, lon, item.url, item.text]);
      });
      // var sql = "INSERT INTO bnas_test (title, language, lat, lon, url, text) VALUES" +
      //   "(\"" + db.title + "\",\"" + db.language + "\"," + lat + "," + lon + ",\"" +
      //   db.url + "\",\"" + db.text.replace(/[\\$'"]/g, "\\$&") + "\");";
    }

    const sql = `INSERT INTO bnas_test (title, language, lat, lon, url, text) VALUES ?`;

    con.query(sql, [itemsArray], function (err, result) {
      if (err) throw err;
      console.log("Table created");
    });
  });
  // con.query("CREATE TABLE bnas_test ( id INT unsigned NOT NULL AUTO_INCREMENT, " +
  //   "title VARCHAR(150), language VARCHAR(5), lat FLOAT(16, 14), lon FLOAT(16, 14), " +
  //   "url VARCHAR(50), text TEXT(20000), PRIMARY KEY (id) );", function (err, result) {
  //     if (err) throw err;
  //     console.log("Table created");
  // });

  //CREATE TABLE bnas_test ( id INT unsigned NOT NULL AUTO_INCREMENT, title VARCHAR(150), language VARCHAR(5), lat FLOAT(16, 14), lon FLOAT(16, 14), url VARCHAR(50), text VARCHAR(10000), PRIMARY KEY (id) );
});

/*INSERT INTO ITEMS (title, language, lat, lon, url, text) VALUES
(1, "Story","mixed",53.0, -7.0, "example.com", "There was a man once who said bail ó dhia ort to people."),
(2, "Duine a chonaic mé","ga",54.0, -6.0, "example.org", "Chonaic mé fear uair amháin agus bhí neart eolais aige."),
(3, "My local district","en",52.0, -6.5, "example.ie", "There is lots of cool stuff in my district.");*/