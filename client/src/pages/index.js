...
import gaeltachtCurrent from 'public/gaeltacht_current.geojson';
...
export default function Home() {
  ...
  const [showGaeltachtCurrent, setShowGaeltachtCurrent] = useState(false);
        ...
        <div class="left-div">
          <form>
            {inputTexts.map((inputText, i) => (<><input
              ...
              onChange={(e) => setInputTexts(previousInputTexts => [...previousInputTexts.slice(0, i), e.target.value, ...previousInputTexts.slice(i + 1)])} /><br /></>))}
            ...
            <div style={{margin: "5px", padding: "5px"}}>
              ...
              <br />
              <input type="checkbox" onChange={(e) => setShowGaeltacht1926(!showGaeltacht1926)} checked={showGaeltacht1926} />
              1926
              &nbsp;&nbsp;
              <input type="checkbox" onChange={(e) => setShowGaeltachtCurrent(!showGaeltachtCurrent)} checked={showGaeltachtCurrent} />
              1956 (current)
            </div>
          ...
          <table style={{padding: "10px"}}><tbody>
            {inputTexts.map((inputText, i) => (
              <tr style={{ width: "50em", color: markerColours[i % 10]}}><td>{markerSymbols[i % 10]}&nbsp;&nbsp;</td><td>{inputText}</td></tr>
            ))}
          </tbody></table>
          <div style={{position: "absolute", bottom: 0, margin: "10px"}}>© 2023 National Folklore Collection, UCD</div>
        </div>
        <div class="right-div">
          <Section>
            <Container>
              <Map className={styles.homeMap} width="750" height="500" center={DEFAULT_CENTER} zoom={7}>
                {({ TileLayer, Marker, Popup, GeoJSON }) => (
                  <>
                    ...
                    {showGaeltacht1926 && <GeoJSON data={gaeltacht1926} color="#800080" fill={false}/>}
                    {showGaeltachtCurrent && <GeoJSON data={gaeltachtCurrent} color="#ff00ff" fill={false}/>}
                  </>
                )}
              </Map>
            </Container>
